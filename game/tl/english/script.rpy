# TODO: Translation updated at 2020-08-09 22:26

# game/script.rpy:121
translate english start_60285286:

    # a "Вчера полиция обнаружила моего деда мертвым."
    a "The police found my grandad’s body yesterday."

# game/script.rpy:122
translate english start_d3028697:

    # a "Испытываю ли я по этому поводу какие-то чувства? Нет."
    a "Do I feel anything about this? Absolutely not."

# game/script.rpy:123
translate english start_ca3b99cf:

    # a "Мой дед был великим писателем. Славу ему принесла публикация первой книги, а затем последовали сплошные творческие неудачи. Критики и пресса издевались над ним и его новыми нелепыми романами."
    a "MMy grandad seemed to be a well-known writer. He got his fame from his first published book but then only failures followed and overfilled the record of his career. Critics and journalists taunted him for his new ludicrous novels."

# game/script.rpy:124
translate english start_eb99dd1b:

    # a "Одни писали, что всему виной девочка-хиппи и наркота, а другие сплетничали о тяжелом алкоголизме."
    a "Some said that it’s a mysterious hippie girl and her drugs to blame. Others spread rumours about chronic heavy drinking."

# game/script.rpy:125
translate english start_25c69828:

    # a "Мне, если честно, все равно."
    a "Honestly, I don’t care."

# game/script.rpy:129
translate english start_eaca051c:

    # a "Я не видел его с самого детства. Все воспоминания о нем - светлые,{w} но в детстве все кажутся хорошими."
    a "I haven’t seen him since my early childhood. I recall only bright memories about him,{w} but childhood is a sleep of reason."

# game/script.rpy:130
translate english start_b4fedbd5:

    # a "Факт в том, что дед бросил нас с матерью, когда нам было очень тяжело после смерти отца. Мама не говорила мне, но она часто писала деду, просила о помощи."
    a "In fact, grandfather abandoned us when we were ruined after papa’s death. Mom didn’t tell me but she used to write to him asking for help."

# game/script.rpy:131
translate english start_9118ec42:

    # a "Он не снизошел."
    a "Silence was the answer."

# game/script.rpy:132
translate english start_a00b51d7:

    # a "Не соизволил даже ответить."
    a "He didn’t deign to even just reply."

# game/script.rpy:133
translate english start_20cfbeb0:

    # a "А я, видимо, слишком быстро подрос и перестал напоминать милого питомца. Родство ведь не повод для сострадания?"
    a "Apparently, I was growing up too fast and no longer a nice pet. Kinship is no more reason for sympathy, isn’t it?"

# game/script.rpy:139
translate english start_ac0fbbd1:

    # a "Теперь я - единственный наследник этого... человека. После смерти матери меня в этой дыре ничего не держит."
    a "Now I’m the only heir to this...man. Well, after mom’s died I have nothing in this godforsaken town..."

# game/script.rpy:140
translate english start_56b4c0f3:

    # a "Кроме плохих воспоминаний."
    a "..but bad memories."

# game/script.rpy:141
translate english start_80b7fcce:

    # a "Надо скорее продать этот мрачный особняк и уехать. Думаю, этих денег должно хватить, чтобы начать новую жизнь. Всегда хотел погрузиться в египтологию."
    a "I wanna sell this dreary mansion and leave for a new life. I bet it’ll be enough money. I’ve always dreamt about egyptology."

# game/script.rpy:146
translate english start_0ae9bcd0:

    # ""
    ""

# game/script.rpy:148
translate english start_15bbf8a2:

    # a "Алло."
    a "Hello?"

# game/script.rpy:149
translate english start_3af3e8cf:

    # i "Артур, я звоню выразить свои соболезно..."
    i "Artur, I call you to give my deepest condolen..."

# game/script.rpy:150
translate english start_49e43b8c:

    # a "Вы звоните не за этим."
    a "You’re calling me for something else."

# game/script.rpy:151
translate english start_f6383cbe:

    # i "Да, тебя не проведешь."
    i "Well, yeah, you see the point."

# game/script.rpy:152
translate english start_93692b43:

    # i "В общем, слушай, за год до своей кончины твой дед Николас сделал заявление в прессе. Мол, теперь-то у него точно будет шедевр."
    i "Listen. Your grandad Nicholas made a promise in the interview to release a true masterpiece about a year ago."

# game/script.rpy:153
translate english start_16fdaeeb:

    # i "Мы в нашем бизнесе таким хвастливым обещаниям верить не привыкли - особенно от тех, кто не оправдывает доверие. Но и упускать возможную прибыль мы себе позволить не можем."
    i "You know, our business prevents us from trusting bragging. Especially, when a bragger has already broken faith. But we can’t pass by potential profits either."

# game/script.rpy:154
translate english start_250f6bd5:

    # i "Год - хороший срок, чтобы довести книгу до ума."
    i "A year is a good period to polish a book."

# game/script.rpy:155
translate english start_7d8ecca3:

    # i "Да и знаешь ли, книги авторов из свежих могил продаются лучше всего."
    i "And you know, freshly dead authors sell better."

# game/script.rpy:156
translate english start_6069c6d0:

    # i "В общем, найдешь рукопись старика - получишь отличный гонорар. Если что, я на связи."
    i "So, if you find your grandad’s writings, you’ll get a bundle. Keep in touch."

# game/script.rpy:157
translate english start_03a14c14:

    # a "Это меняет дело. Так денег хватит даже на целую экспедицию..."
    a "Well, it makes a difference. In this case I’ll afford even an expedition."

# game/script.rpy:164
translate english start_530e1347:

    # a "Тут... довольно мило, но довольно пыльно. Надо осмотреться."
    a "Seems quite nice and quite dusty. I’d look around."

# game/script.rpy:168
translate english start_85b1c598:

    # a "Он... хранил нашу фотографию? Зачем? Любовался теми, кого бросил?"
    a "He kept our photo? For what sake? To behold those he abandoned?"

# game/script.rpy:169
translate english start_aa4b3553:

    # a "Похоже, сплетни не врали. Старый развратник. На вид она ничего так... Не ради ли тебя наш дед забыл о нашей семье? Хотя... если он так легко выкинул нас из сердца, то проблема тут явно не в ней..."
    a "Well, rumours occured to be true. Old perv. She’s kind of pretty, though. Did she outshine our family? However, if he threw us away from his heart so easily, then it’s not her fault anyway."

# game/script.rpy:171
translate english start_eec4daa0:

    # a "А там и недопитые бутылки. Значит, и это правда. Вот и истина в вине... Жалкий оказался человек..."
    a "I see half-finished bottles from here. Another truth revealed, another gossip confirmed. In vino veritas. What a despicable man he was..."

# game/script.rpy:175
translate english start_6f5ec3ff:

    # a "Хм... толстый слой пыли на клавишах. Никто не играл на нем уже много лет. Очередной бессмысленный предмет."
    a "Thick dust layer over piano keys. No one played it for decades. Another useless props."

# game/script.rpy:185
translate english start_0ae9bcd0_1:

    # ""
    ""

# game/script.rpy:186
translate english start_09583e24:

    # a "Черт, только этого не хватало. Теперь еще щитки искать..."
    a "Damn, that’s just what I needed. Now I have to to find a fuse box."

# game/script.rpy:196
translate english start_d2ab6966:

    # a "Где же они? Черт!"
    a "Where’s it? Shit!"

# game/script.rpy:202
translate english start_65a1bbc5:

    # a "Что это такое... Здесь никого не должно быть."
    a "What the fuck is it? There should be no one else!"

# game/script.rpy:205
translate english start_2889771f:

    # a "Эй, кто здесь?"
    a "Hey, who’s there?"

# game/script.rpy:221
translate english start_3062847c:

    # a "Это напрягает."
    a "It’s damn creepy."

# game/script.rpy:224
translate english start_ce96d54e:

    # a "Кто, черт возьми, зажег эту свечу?{w} Сюда что, кто-то вломился? Надо звонить копам."
    a "Who the fuck lit the candle? Intruders? Better call the cops."

# game/script.rpy:225
translate english start_ba56eddc:

    # a "Черт, мобильная связь тоже накрылась. Что за жесть такая тут происходит? Лучше уйти отсюда. Возьму свечу, а то с той лестницы можно и навернуться ненароком."
    a "Fuck! Out of coverage. What the fuck is going on here? I’m leaving right now. I’ll take the candle to not fall down the stairs."

# game/script.rpy:233
translate english start_0191aacc:

    # a "Кажется, в каминной на первом этаже кто-то есть. Выйти из дома можно только там. Черт, почему в этом доме нет черного входа?"
    a "It sounds like someone is on the ground floor. In the living room. The only exit is there. Fuck, why is there no backdoor?"

# game/script.rpy:234
translate english start_81b660f4:

    # a "Придется выяснить, кто там бродит. Надеюсь, он один и не вооружен."
    a "I have to deal with the intruder now. Hope he’s alone and unarmed."

# game/script.rpy:241
translate english start_71e87845:

    # n "Как же она красива..."
    n "She’s so charming..."

# game/script.rpy:242
translate english start_e5d5ecd5:

    # a "Эй! Ты кто такой? Как ты сюда забрался вообще? А ну..."
    a "Hey! Who the fuck are you? How did you get here? You bette..."

# game/script.rpy:243
translate english start_2c09ce09:

    # n "Ты когда-нибудь любил?"
    n "Did you ever fall in love?"

# game/script.rpy:244
translate english start_c220bd07:

    # a "Слушай, мне сейчас не до романтики. Сюда уже едет полиция. Медленно подними руки. Это ты вырубил свет?"
    a "Fuck this love stuff, you freak! I’ve called the cops. Hands up, slowly! Is it you who cut off the lights?"

# game/script.rpy:245
translate english start_1de5f2b9:

    # n "Нет, это Грегори. А я - Лайонель, к слову."
    n "No, it was Gregory. And I’m Lionel, nice to meet you."

# game/script.rpy:246
translate english start_620606c5:

    # a "Какой к черту Грегори? Где он?"
    a "Who the fuck is Gregory? Where’s he?"

# game/script.rpy:247
translate english start_12f7b6a2:

    # leo @ ghost vspom "Стоит прямо за тобой."
    leo @ ghost vspom "He’s just behind you?"

# game/script.rpy:258
translate english start_0c105709:

    # a "Твою..."
    a "What the..."

# game/script.rpy:263
translate english start_337fa71e:

    # a "Что это было? Что? Он... он не мог просто исчезнуть, я отвернулся на секунду. Что за?.. Черт, надо убираться отсюда."
    a "It’s not possible... He couldn’t just...disappear. I turned away for a second! Fuck, I need to find my way out!"

# game/script.rpy:272
translate english start_10ca79c7:

    # a "Черт, она заперта! Как? Как это возможно?"
    a "Damn, it’s locked! How come? How is it possible?"

# game/script.rpy:277
translate english start_7bf3eda7:

    # a "Крепкая, выбить почти нереально. Так, надо успокоиться. Паника убивает быстрее всего."
    a "It’s barely impossible to knock it out. I need to calm down. Panic kills."

# game/script.rpy:282
translate english start_0d83c6a4:

    # a "В детстве кто-то учил...{w} кажется на фехтовании...{w} короткий вдох, продолжительный выдох."
    a "I remember...{w} I was a child... someone taught me... short inhale, long exhale. It’s better now."

# game/script.rpy:284
translate english start_75e782bd:

    # a "Надо осмотреть комнаты, может удастся найти ключ."
    a "I should go and look for the key."

# game/script.rpy:291
translate english start_6173e19d:

    # a "Эй, ты кто вообще такой? Что ты тут забыл?"
    a "Hey, who the fuck are you? What do you need here?"

# game/script.rpy:292
translate english start_280c0a4c:

    # n "Тише, тише, молодой человек, не надо так орать, будто вы до мертвых добудиться хотите.{w} Позвольте представиться: Уильям, мастер слова и пера."
    n "Hush, lad, don’t be so loud as if you are shouting at the dead to awake them. Let me introduce myself.{w} I am William, the master of words"

# game/script.rpy:293
translate english start_96c295de:

    # Will "Кстати, как вам этот отрывок:"
    Will "By the way, could you be so kind to deliver your opinion on this:"

# game/script.rpy:294
translate english start_23cb8c9d:

    # Will "«Ход его рассуждений прервался, и он увидел бесконечную тьму ее разума, лишенного всяческого сострадания. Она была безжалостным хищником, мифической сиреной, одержимой неутолимой жаждой губить души...»"
    Will "«His train of thought crashed, so he discerned the infinite darkness in her mind bereft of any sympathy. She was a merciless predator, a siren obsessed with an insatiable desire to feed on ruined souls...»"

# game/script.rpy:295
translate english start_5d62cc48:

    # a "Вы знакомый моего деда Николаса?"
    a "Are you a friend of my grandpa Nicholas?"

# game/script.rpy:297
translate english start_b8946642:

    # Will "Конечно, я знал его всю свою жизнь."
    Will "Certainly, I have known him for my whole life!"

# game/script.rpy:298
translate english start_08a730cb:

    # Will "Ах, бедный, бедный Николас. Какое счастье, что он покинул свою скорбную юдоль позора..."
    Will "Oh, poor Nicholas! What a relief he finally left his mournful vale of disgrace."

# game/script.rpy:299
translate english start_2d891732:

    # a "Тогда, может, поможете мне выйти отсюда?"
    a "Well, maybe you can help me out of here?"

# game/script.rpy:300
translate english start_406d43a4:

    # Will "Выйти? Так спешите в суету мира, молодой человек? А могли бы испить из источника подлинного творчества."
    Will "Out? You are rushing to the commotions of the world, aren’t you? Instead, you could drink up the cup of the genuine art."

# game/script.rpy:301
translate english start_189f25f5:

    # Will "Как прискорбно, еще один обыватель, а ведь все-таки внук Николаса."
    Will "It is so sorrowful, another philistine I come across... And yet he is a grandchild of Nicholas."

# game/script.rpy:302
translate english start_1c428ff9:

    # a "Эй, откуда вы знаете?"
    a "Hey, how do you know?"

# game/script.rpy:303
translate english start_96f4e657:

    # Will "Ключ вон там, на стене, видите? Не смею задерживать."
    Will "The key is there, on the wall. I do not want to bother you any further."

# game/script.rpy:307
translate english start_7cb95a3d:

    # a "Но... там нет никакого ключа!"
    a "But... there’s no key!"

# game/script.rpy:312
translate english start_7f1cbdbf:

    # a "Ты еще кто такой?"
    a "Hey, who the fuck are you?"

# game/script.rpy:313
translate english start_5d73183f:

    # g "Я - Грегори, а ты - мерзкий вор."
    g "I am Gregory and you are a filthy burglar."

# game/script.rpy:314
translate english start_ac2ab2af:

    # g "Думал, раз людишки вроде тебя свели Николаса в могилу, то ты можешь спокойно шнырять по его дому?"
    g "You thought what? You got tired of dancing on Nick’s grave and came here to dart around in his house?"

# game/script.rpy:315
translate english start_7bed0c40:

    # g "Позарился на чужое?"
    g "You wanted to steal his belongings?!"

# game/script.rpy:316
translate english start_2e9fe78d:

    # g ghost smile "Убирайся отсюда!"
    g ghost smile "Get the fuck out of here!"

# game/script.rpy:331
translate english start_0ae9bcd0_2:

    # ""
    ""

# game/script.rpy:339
translate english start_f6da3028:

    # a "Я что, схожу с ума?! Этого не может быть... Это все нереально."
    a "Am I going crazy right now?! It’s impossible... It can’t be real."

# game/script.rpy:341
translate english start_a6043b77:

    # a "Боже, как же мне холодно... какой-то озноб...{w} надо сначала... срочно найти что... чем согреться."
    a "God, it is so cold... some shiver... I need to... find something... to get warm."

# game/script.rpy:353
translate english start_08768294:

    # a "Хорошо, вроде получше... не так холодно."
    a "Well, that’s much better."

# game/script.rpy:354
translate english start_a6af7fec:

    # a "Все не могу понять:{w} я брежу? Я сплю?{w} Или с этим местом что-то не так?"
    a "I still can’t understand.{w} Am I hallucinating? Am I sleeping? What’s wrong with this place?"

# game/script.rpy:355
translate english start_837c49bd:

    # a "Они же выглядит неживыми...{w} гниющими?"
    a "These disappearing men look so weird... so lifeless{w} and maybe even rotting..."

# game/script.rpy:356
translate english start_9ea55ca1:

    # a "Они прокляты? Я проклят? Черт..."
    a "Are they cursed? Am I cursed? Damn..."

# game/script.rpy:358
translate english start_9531549d:

    # a "Тут среди углей были какие-то обгоревшие бумаги."
    a "The fireplace kept the remains of something like burnt papers."

# game/script.rpy:364
translate english start_276f2a44:

    # a "Любопытно. Значит, дед был серьезно болен... Это, конечно, его не оправдывает, но..."
    a "Curious. So, grandad was really ill... Well. it doesn’t justify him, but..."

# game/script.rpy:365
translate english start_bee7bd21:

    # a "И что, эти его субличности смогли как-то воплотиться после смерти? Жуть какая."
    a "And his subpersonalities have managed somehow to incarnate after his death? Gross!"

# game/script.rpy:369
translate english start_b8db4c30:

    # a "Похоже, и вправду попал в дом с привидениями. Или в готический роман. Эдгар, ты балуешься?"
    a "Seems like I found myself in a haunted mansion. Straight from a gothic fiction. Edgar, is it you making fun of me?"

# game/script.rpy:370
translate english start_f0598759:

    # a "Ха. А дед и из могилы смог доставить хлопот. Но, похоже, сошел с ума все-таки не я."
    a "Huh. And my grandad managed to cause troubles for me even from the grave. At least, it’s not me who’s gone insane."

# game/script.rpy:371
translate english start_bdb411d1:

    # a "Ну, видимо, все по классике: разгадай загадку дома, чтобы выйти.{w} Надеюсь, бледная хрупкая красотка по канону жанра тоже будет."
    a "I guess it’s like in the classics. I need to solve a riddle of the mansion to find a way out. I hope there’ll also be a pale delicate lady."

# game/script.rpy:372
translate english start_342d702b:

    # a "Пойду в кабинет, может найду записи.{w} Надо взять свечу."
    a "I’ll go to the study and look for some other records.{w} It’s better to take a candle with me."

# game/script.rpy:383
translate english start_c2f30e31:

    # Will "О, вы вовремя! Сейчас, послушайте монолог моего героя о тщетности бытия! Только что написал."
    Will "Oh, you are right in time! Please, listen to the monologue of my character about the vanity of existence that I have just written."

# game/script.rpy:391
translate english start_8050a6fc:

    # Will ghost "Это кто? Имена будто у персонажей каких-то второсортных новелл."
    Will ghost "Who are them? These names are likely suitable for characters of low-tier novels."

# game/script.rpy:395
translate english start_a06c43f7:

    # Will ghost "Молодой человек, мой ум постоянно вовлечен в сложную интеллектуальную деятельность."
    Will ghost "You know, my mind is permanently engaged in sophisticated intellectual activity."

# game/script.rpy:396
translate english start_a7efc82c:

    # Will ghost "Мне некогда рассматривать каких-то девиц. Даже на портретах."
    Will ghost "I have neither time, nor will to look at any girls. Even if they are in pictures."

# game/script.rpy:399
translate english start_8eeb54d9:

    # n "О, вы вовремя! Сейчас, послушайте монолог моего героя о тщетности бытия! Только что написал."
    n "Oh, you are right in time! Please, listen to the monologue of my character about the vanity of existence that I have just written."

# game/script.rpy:403
translate english start_9d345861:

    # n "Мне кажется, любой обладатель мало-мальски среднего интеллекта уже бы сообразил, что я писатель."
    n "It seems to me that even an underdeveloped intelligence would have figured out that I am a writer."

# game/script.rpy:404
translate english start_e80a2d5a:

    # n "Меня зовут Уильям."
    n "My name is William."

# game/script.rpy:405
translate english start_fc540512:

    # Will "Близкий друг Николаса, некогда - хозяина этого прекрасного кабинета.{w} К вашим услугам."
    Will "A close friend of Nicholas, once a host of this gorgeous cabinet. At your service."

# game/script.rpy:412
translate english start_2d5d6f31:

    # Will ghost ob "Это кто? Имена будто у персонажей каких-то второсортных новелл."
    Will ghost ob "Who are them? These names are likely suitable for characters of low-tier novels."

# game/script.rpy:416
translate english start_4baaa638:

    # Will ghost ob "Молодой человек, мой ум постоянно вовлечен в сложную интеллектуальную деятельность."
    Will ghost ob "You know, my mind is permanently engaged in sophisticated intellectual activity."

# game/script.rpy:417
translate english start_8b2d4f95:

    # Will ghost ob "Мне некогда рассматривать каких-то девиц. Даже на портретах."
    Will ghost ob "I have neither time, nor will to look at any girls. Even if they are in pictures."

# game/script.rpy:427
translate english start_0f4d74ed:

    # Will @ ghost o4ki "Молодой человек! Николас хоть и был посредственным писателем, но человеком был весьма приличным."
    Will @ ghost o4ki "Be respectful! Obviously, Nicholas has killed the art of literature with every book he wrote but he was a decent man, not a murderer."

# game/script.rpy:428
translate english start_0c4bc478:

    # Will @ ghost o4ki "Как вы вообще могли такое о нем подумать?"
    Will @ ghost o4ki "How did these thoughts ever come to your mind?"

# game/script.rpy:432
translate english start_fd48af62:

    # Will "Молодой человек, не разочаровывайте меня окончательно."
    Will "Please, do not disappoint me completely!"

# game/script.rpy:433
translate english start_237be041:

    # Will "Писательский труд суть чистое творение разумом вселенной."
    Will "Writing is a creation of the new universe by your word."

# game/script.rpy:434
translate english start_9e05eff4:

    # Will "Вы чувствуете метафизическое очарование подобного определения?"
    Will "Do you feel a metaphysical fascination with the definition given?"

# game/script.rpy:435
translate english start_1e286733:

    # Will "Кто знает, быть может и мы - всего лишь персонажи чьей-то пьесы или прозы и говорим лишь то, что кто-то неведомый нам в уста вложил."
    Will "Who knows, maybe even we are nothing but characters of someone’s play and utter the words someone’s written for us."

# game/script.rpy:441
translate english start_544051a5:

    # Will "Да, Николас успел закончить свой скорбный и не самый изысканный труд."
    Will "Yes, Nicholas managed it. Of course, the book can be barely called exquisite."

# game/script.rpy:458
translate english two1_4b27e0fa:

    # Will "Однако, вы умудрились истощить мое долгое терпение и окончательно меня разочаровать."
    Will "Nonetheless, you managed to exhaust my patience and disappoint me completely."

# game/script.rpy:459
translate english two1_a16e8c35:

    # Will "Вы могли бы прочесть здесь произведения истинного гения, но предпочли писанину вашего деда."
    Will "You could read masterpieces of a true genius here, but you preferred to stick to your grandad’s scribbles."

# game/script.rpy:460
translate english two1_79bb11a7:

    # Will "Что ж, прах к праху. Признаюсь, когда я прочитал последний труд вашего деда, я ужаснулся.{w} Из всех книг, что он написал - это худшая, без сомнения."
    Will "Ashes to ashes. I have to confess. When I read the last book of your grandad, it terrified me. It was the worst thing I had ever read."

# game/script.rpy:461
translate english two1_692cea0c:

    # Will "Ради его репутации, я ее немножко подправил, точнее, основательно переписал."
    Will "For the sake of his reputation, already spoiled though, I totally rewrote the book."

# game/script.rpy:462
translate english two1_54bbb0cb:

    # Will "Я отдам ее вам, но исключительно для того, чтобы вы оставили меня в покое."
    Will "I am going to give you it, but, in turn, you should leave me alone."

# game/script.rpy:463
translate english two1_b763ecc9:

    # Will @ ghost ob "Берите и идите, прошу вас! Продайте ее и живите сытой жизнью за счет плодов чужого ума, как и полагается людям вашего сорта."
    Will @ ghost ob "Please, take it and leave! Sell it and live the life of philistine at the cost of the fruits of other’s mind."

# game/script.rpy:464
translate english two1_985d1882:

    # Will @ ghost ob "Прощайте."
    Will @ ghost ob "Farewell."

# game/script.rpy:474
translate english two1_93d94966:

    # a "Тут... одно и то же..."
    a "It’s all the same..."

# game/script.rpy:480
translate english two1_2b9685f8:

    # a "По нему я точно не буду скучать. Он действовал мне на нервы."
    a "I’ll hardly miss him. He was really getting on my nerves."

# game/script.rpy:481
translate english two1_cb43fc32:

    # a "Теперь надо разобраться с другими осколками непутевого деда."
    a "Let’s deal with other shards of my grandpa."

# game/script.rpy:492
translate english two2_44fd188c:

    # Will "Я понимаю, сложно смотреть объективно из-за уз родства,{w} но прошу вас, не стоит быть таким наивным."
    Will "I realize, it is hard to see things impartially due to bonds of kinship. But, please, don’t be so naive."

# game/script.rpy:493
translate english two2_ef25cfc8:

    # a "Дед написал минимум один бестселлер, общепризнанно вошедший в классику литературы."
    a "My grandpa has written at least one bestseller that was generally recognized."

# game/script.rpy:494
translate english two2_d1f0cbfb:

    # Will "Там вот какая прелюбопытнейшая история."
    Will "Oh, there was a curious story with it."

# game/script.rpy:495
translate english two2_1e006376:

    # Will "Видите ли, у Николаса был дед, предыдущий хозяин этого дома."
    Will "You know, the grandfather of Nicholas was the previous owner of this mansion."

# game/script.rpy:496
translate english two2_b732eb80:

    # Will "Говорили, он был буйнопомешанным и даже зарезал какую-то девчонку.{w} А еще он был писателем-неудачником."
    Will "They said he was crazed and even stabbed some girl.{w} And he also was an unfortunate writer."

# game/script.rpy:497
translate english two2_b8835984:

    # Will "Однако, в конце жизни он все же смог написать нечто шедевральное.{w} Но издать не успел."
    Will "However, in the end of his life he managed to write something really awesome{w} but had insufficient time to publish it."

# game/script.rpy:498
translate english two2_d545510f:

    # Will "И Николас, тогда еще молодой, получил его книгу в наследство."
    Will "And our dear young Nicholas inherited this book."

# game/script.rpy:499
translate english two2_02ac4767:

    # Will "Близким друзьям он говорил, что книгу ему вручил призрак...{w} Чушь для впечатлительных дам и легковерных глупцов!"
    Will "Well, he confided a story about ghosts giving him this book to his close friends. Absolute nonsense! Ghosts do not exist!"

# game/script.rpy:500
translate english two2_8bc84248:

    # Will "Недолго думая, ваш дед присвоил себе авторство, став в момент известным и богатым."
    Will "Without thinking twice, Nicholas arrogated authorship and in no time became rich and famous."

# game/script.rpy:501
translate english two2_b793490c:

    # Will ghost o4ki "Вот только сам он уже не смог создать что-либо стоящее."
    Will ghost o4ki "But the truth is that he never created on his own anything worth paper he spoiled galore."

# game/script.rpy:502
translate english two2_2d70b652:

    # Will "Я, конечно, многому пытался его научить, но, увы, его потолок - бульварные детективчики."
    Will "Definitely, I tried to teach him, but, unfortunately, he was not able to reach beyond tabloid stories."

# game/script.rpy:503
translate english two2_cc04554b:

    # a "Интересная история. Скажите, а что издали вы?"
    a "It is a fairly interesting story. But, please, tell me, what have you published?"

# game/script.rpy:504
translate english two2_ea776d8b:

    # Will "Помилуйте, как некультурно спрашивать такое..."
    Will "Forgive me, it is so ill-mannered of you to ask such..."

# game/script.rpy:505
translate english two2_0c14c4f3:

    # a "И все же, я настаиваю."
    a "But still, I insist!"

# game/script.rpy:506
translate english two2_8d9c6bcd:

    # Will "Подите прочь..."
    Will "Please, leave me alone..."

# game/script.rpy:507
translate english two2_fb2baed8:

    # a "Вы ничего никогда не издавали и не писали."
    a "You have written and published nothing."

# game/script.rpy:508
translate english two2_0c6d4cfa:

    # a "Ведь вы - всего лишь маска.{w} Вы - иллюзия гениальности."
    a "As you are nothing but a mask.{w} You are a mere illusion of brilliance."

# game/script.rpy:509
translate english two2_08675e1d:

    # a "Вас создал мой дед и создал весьма занудным.{w} Ваша роль - быть барьером от страшной для него правды. Той правды, что он никогда не был достоин этого статуса и славы."
    a "My grandad created you. You stand as a barrier defending from the frightening truth that he never deserved his titles and fame."

# game/script.rpy:510
translate english two2_7b42bcb2:

    # a "Он умер, а вы так и остались его марионеткой.{w} Как вам такой поворот сюжета?"
    a "And then he died but even in death you remained his puppet. How do you like this plot twist?"

# game/script.rpy:511
translate english two2_dd022427:

    # Will ghost "Вы... вы... правы."
    Will ghost "You... You are right. Alas!"

# game/script.rpy:512
translate english two2_76d66a7b:

    # Will "Да, теперь я вижу всю картину целиком."
    Will "Now I see the whole picture."

# game/script.rpy:513
translate english two2_981da51a:

    # Will "Прискорбно признавать, но похоже, я - лишь красивая обложка, за которой тщетная пустота."
    Will "Regrettably, I have to recognize that I am a beautiful cover with nothing underneath."

# game/script.rpy:514
translate english two2_4df86fa9:

    # Will "Это так больно.{w} Но в то же время...{w} Я чувствую, что наконец-то свободен."
    Will "It is so painful...{w} However, at the same time, I finally feel free."

# game/script.rpy:515
translate english two2_9c73257e:

    # Will "Парадоксально, но я благодарен вам.{w} Без вас, кто знает, сколько бы я еще был прикован к этому месту."
    Will "Paradoxically, I am grateful to you.{w} Who knows how many centuries I would have spent here chained to the place if it was not you."

# game/script.rpy:516
translate english two2_bb7d1beb:

    # Will "Прощайте."
    Will "Farewell."

# game/script.rpy:520
translate english two2_2b9685f8:

    # a "По нему я точно не буду скучать. Он действовал мне на нервы."
    a "I’ll hardly miss him. He was really getting on my nerves."

# game/script.rpy:521
translate english two2_cb43fc32:

    # a "Теперь надо разобраться с другими осколками непутевого деда."
    a "Let’s deal with other shards of my grandpa."

# game/script.rpy:532
translate english two3_b7af8680:

    # Will "Дешевая интрига, молодой человек.{w} Дешевая интрига и неподобающий тон."
    Will "It is a fairly lowbrow suspense, sir."

# game/script.rpy:533
translate english two3_7e2e73e0:

    # Will "Ну что же, попробуйте, поразите меня."
    Will "But still, please, try to astonish me."

# game/script.rpy:534
translate english two3_f6ce2e2f:

    # a "Ты - просто призрак моего выжившего из ума деда Николаса."
    a "You are nothing but a ghost of my crazy grandad Nicholas."

# game/script.rpy:535
translate english two3_07098fb3:

    # Will "Это возмутительно!{w} Как вы смеете! Вы перешли все границы!"
    Will "It is scandalous! How dare you! You crossed the line!"

# game/script.rpy:536
translate english two3_b30a495b:

    # Will "Убирайтесь отсюда, я больше не собираюсь лицезреть вашу отвратительную физиономию."
    Will "Get out! I can not endure your presence any longer!"

# game/script.rpy:540
translate english two3_c15ce801:

    # a "По нему я точно не буду скучать.{w} Он действовал мне на нервы."
    a "I’ll hardly miss him. He was really getting on my nerves."

# game/script.rpy:541
translate english two3_cb43fc32:

    # a "Теперь надо разобраться с другими осколками непутевого деда."
    a "Let’s deal with other shards of my grandpa."

# game/script.rpy:553
translate english three_d6c523ee:

    # n "Такая изысканная шпага так долго лежала без дела.{w} Это неправильно."
    n "This sword is so exquisite. And yet it’s lying idle. What a pity."

# game/script.rpy:554
translate english three_bf6aacea:

    # n "Оружие должно отнимать жизни.{w} А этой шпагой я уже давно никого не убивал."
    n "The blade must take lives.{w} But with this sword I killed no one for so long time."

# game/script.rpy:555
translate english three_31f09d8e:

    # n "Но, вижу, сегодня настал ее час.{w} Страшно?{w} Страшно, я же вижу."
    n "But now I see:{w} its time has come.{w} Are you afraid?{w} I know you are."

# game/script.rpy:556
translate english three_efa885e7:

    # a "Ты кто?"
    a "Who are you?"

# game/script.rpy:557
translate english three_7ec4c9b4:

    # n "Я? Меня зовут Грегори, неприятно познакомиться."
    n "Me? I am Gregory, unpleasant to meet you."

# game/script.rpy:558
translate english three_80eb16e2:

    # g ghost evil smile "Я - уничтожитель всего, что смело когда-либо причинять боль несчастному Николасу."
    g ghost evil smile "I was an eradicator of everything that inflicted pain on poor Nicholas."

# game/script.rpy:559
translate english three_0842cebd:

    # g "Теперь я охраняю его наследство.{w} А еще я твой будущий убийца."
    g "And now I am guarding his inheritance. Also, I am your future murderer."

# game/script.rpy:561
translate english three_3030520f:

    # g "Такая изысканная шпага так долго лежала без дела. Это неправильно."
    g "This sword is so exquisite. And yet it’s lying idle. What a pity."

# game/script.rpy:562
translate english three_d22e98f1:

    # g "Оружие должно отнимать жизни. А этой шпагой я уже давно никого не убивал."
    g "The blade must take lives. But with this sword I killed no one for so long time."

# game/script.rpy:563
translate english three_7c8b53fb:

    # g "Но, вижу, сегодня настал ее час. Страшно? Страшно, я же вижу."
    g "But now I see: its time has come. Are you afraid? I know you are."

# game/script.rpy:564
translate english three_430ef887:

    # g ghost enemy "Ну давай, закрой глазки ручками, это поможет..."
    g ghost enemy "Come on, close your eyes, coward, it really helps."

# game/script.rpy:568
translate english three_dc8ee85d:

    # g ghost enemy "Сейчас ты заплатишь за эти слова! Защищайся!"
    g ghost enemy "You’ll pay for these words! En garde!"

# game/script.rpy:587
translate english three_1c1c21fd:

    # g ghost enemy "Да, какая-то тварь не отсюда. Пыталась предать Николаса, плела тут вокруг свои сети коварства."
    g ghost enemy "Yeah, it was something not from here. She attempted to betray Nicholas and spinned a web of lies."

# game/script.rpy:588
translate english three_490e7412:

    # g "Я даже не потрудился запомнить, как ее звали."
    g "I didn’t even bother to remember her name."

# game/script.rpy:589
translate english three_182f92f4:

    # g "Думаю, тебя я тоже не запомню..."
    g "I think I won’t remember you either."

# game/script.rpy:590
translate english three_b07a97ea:

    # a "Ты убил... женщину?"
    a "You killed... a woman?"

# game/script.rpy:591
translate english three_b03585db:

    # g "Если ты еще хотя бы слово о ней скажешь, я отправлю тебя прямо к ней.{w} Там ты вдоволь насытишь свое любопыство..."
    g "If you say another word about her, I’ll send you down to her. There you’ll satisfy your obnoxious curiosity."

# game/script.rpy:595
translate english three_f7858457:

    # g ghost smile "Надо же. Внук, наследник..."
    g ghost smile "Wow. Grandson, heir..."

# game/script.rpy:596
translate english three_c0b4b693:

    # g "Где же ты был, внук, когда твоему деду было так плохо?{w} Когда все ополчились против него?{w} Отчего не навестил?"
    g "Where were you, grandson, when your grandfather suffered? When did everyone hate him? Why didn't you come?"

# game/script.rpy:597
translate english three_6f4aee89:

    # g "Зато стоило миленькому дорогому особнячку освободиться, и наш герой уже здесь."
    g "But as soon as a nice expensive mansion gets vacant, our hero is already here."

# game/script.rpy:598
translate english three_8db88f5a:

    # g ghost evil smile "Что ж, прошу вас, наследник, возьмите то, что вам причитается. Удачного владения!"
    g ghost evil smile "Well, please, heir, take what belongs to you. Happy ownership!"

# game/script.rpy:602
translate english three_da8ea192:

    # a "Да он просто издевается!"
    a "He's just fucking mocking me!"

# game/script.rpy:606
translate english three_fa4dadb6:

    # g ghost evil smile "Что ты знаешь вообще?"
    g ghost evil smile "What do you know anyway?"

# game/script.rpy:607
translate english three_a6ddb24a:

    # a "Знаю то, что ты - лишь инструмент. Как эта шпага."
    a "I know that you are just an instrument, like this sword."

# game/script.rpy:608
translate english three_01e71b2f:

    # a "Дед пользовался тобой, чтобы и дальше себя жалеть."
    a "Grandpa used you to continue indulging himself."

# game/script.rpy:610
translate english three_ac55c582:

    # a "За твоей агрессией так удобно было прятать слабость."
    a "It was so convenient for him to hide his weakness behind your aggression."

# game/script.rpy:611
translate english three_41934ad4:

    # a "Из-за тебя его все бросили, ты всех распугал, а дед выл от одиночества."
    a "Because of you, everyone left him, you scared everyone off, and the grandpa howled from loneliness for years."

# game/script.rpy:612
translate english three_2721609c:

    # a "Так кого ты защитил, Грегори?{w} И от того ли надо было защищать?"
    a "So who did you protect, Gregory? And was that even necessary to protect?"

# game/script.rpy:613
translate english three_60e353c7:

    # a "Ты не справился."
    a "You failed."

# game/script.rpy:614
translate english three_d8423c21:

    # a "Ты должен был научить деда защищаться самостоятельно."
    a "You should have taught my grandpa to defend himself."

# game/script.rpy:615
translate english three_2f32598c:

    # g ghost smile "Ты... я ненавижу тебя, но ты, черт возьми, прав."
    g ghost smile "You... I hate you, but you're damn right."

# game/script.rpy:616
translate english three_3c7ca212:

    # g "Спасибо.{w} Ты показал мне правду, и теперь я свободен."
    g "Thank you.{w} You showed me the truth and now I am free."

# game/script.rpy:617
translate english three_ee5fde59:

    # g "Ты будешь хорошим наследником."
    g "You’ll be a good heir."

# game/script.rpy:618
translate english three_f68523ea:

    # g "Надеюсь, ты найдешь книгу Николаса, и все войдет в привычную колею."
    g "I hope you’ll find Nicholas's book and everything goes back to the track."

# game/script.rpy:619
translate english three_590c684c:

    # g "Для меня было честью встретить тебя,{w} хотя для нас обоих эта встреча и не была приятной."
    g "It was an honor for me to meet you,{w} although this meeting was not pleasant for both of us."

# game/script.rpy:620
translate english three_de33db48:

    # g normal "Прощай."
    g normal "Farewell."

# game/script.rpy:635
translate english deadone_313c42e5:

    # "Вы не успели уклониться. Похоже, шпага пронзила ваше сердце."
    "It looks like you didn't manage to dodge. The sword pierced your heart."

# game/script.rpy:645
translate english for_31c4a14f:

    # a "Он чуть меня не убил. Бешеный ублюдок!"
    a "He almost killed me. Mad bastard!"

# game/script.rpy:646
translate english for_8685c4f3:

    # a "Сам не ожидал, что смогу так ловко увернуться. Как будто в детстве меня учили пируэтам. Может дед и учил? И этим спас меня от своей же агрессивной части?"
    a "I did not expect that I could dodge so deftly. As if I was taught this as a child. Maybe  grandfather taught? And by this, he saved me from his aggressive part? Ironically."

# game/script.rpy:647
translate english for_dc3dc7c9:

    # a "Ладно, черт с этим Грегори, пойду в каминную, может третий будет адекватнее."
    a "Okay, fuck that Gregory, I'll go to the living room, maybe the third one will be more adequate."

# game/script.rpy:651
translate english for_c61c6149:

    # a "Ну, хотя бы не пырнули."
    a "Well, at least I wasn't stabbed."

# game/script.rpy:652
translate english for_f7ec812f:

    # a "Пора ставить точку в этой затянувшейся драме."
    a "It's time to put an end to this drawn-out drama."

# game/script.rpy:653
translate english for_f036e928:

    # a "Третий должен быть в каминной, я его видел именно там."
    a "The third one should be in the living room, I’ve seen him there."

# game/script.rpy:654
translate english for_8e55e6cb:

    # a "Черт, призраки, привязанные к одному месту - какое клише!"
    a "Damn, ghosts bounded to one place - such a cliché!"
    
# game/script.rpy:660
translate english for_452646e3:

    # leo "Так странно.{w} Этот камин не разжигали уже очень давно."
    leo "So weird. This fireplace hasn't been lit for a long time."

# game/script.rpy:661
translate english for_585efed7:

    # leo @ ghost vspom "Я все-таки узнал тебя.{w} Ты - внук Николаса, верно?"
    leo @ ghost vspom "I recognize you. You're Nicholas's grandson, aren’t you?"

# game/script.rpy:662
translate english for_8714ef24:

    # leo "Я... он испытывает к тебе сильную привязанность.{w} Ты должен уйти."
    leo "I... he has a strong affection for you. You have to leave."

# game/script.rpy:663
translate english for_71e90170:

    # leo "Пойми, Николас... мы не можем, не должны находиться рядом с теми, кто нам дорог. Поэтому я просил Николаса прекратить общение с тобой."
    leo "Understand, Nicholas we cannot, should not be near those who are dear to us. So I asked Nicholas to stop communicating with you."

# game/script.rpy:664
translate english for_eb0dd1dc:

    # leo "Мы разрушаем все, что любим. Мы... я - настоящее чудовище..."
    leo "We destroy everything we love. We are... I am a real monster..."

# game/script.rpy:668
translate english for_7f903578:

    # a "Дед, мне надоели твои игры и не нужны твои оправдания."
    a "Grandfather, I'm tired of your games and I don't need your excuses."

# game/script.rpy:669
translate english for_b8527e09:

    # a "Ты оставил меня, забыв о моих чувствах, поэтому не требуй от меня заботы о твоих."
    a "You left me and by doing so expressed no respect for my feelings, so don’t demand to take care of yours."

# game/script.rpy:670
translate english for_c7661d0f:

    # a "Тебя никогда не интересовал кто-либо кроме себя.{w} Ты и сюда прибежал, чтобы жалеть себя."
    a "You have never been interested in anyone except yourself. You even came here to nourish self-pity."

# game/script.rpy:671
translate english for_08c5e2d5:

    # a "Ты сам во всем виноват и заслужил все, что с тобой произошло."
    a "You are to blame for everything and deserve everything that happened to you."

# game/script.rpy:672
translate english for_bb1d1ba9:

    # leo "Ты прав. Абсолютно."
    leo "You're right. Absolutely."

# game/script.rpy:675
translate english for_23c7a091:

    # leo "Я эгоист. Я ничтожество. Я убийца. Я заслужил муки."
    leo "I'm selfish. I'm miserable. I am a murderer. I deserve to be tormented."

# game/script.rpy:676
translate english for_92499eae:

    # leo "Пора перестать прятаться и принять свою участь в адском пламени."
    leo "It's time to stop hiding and accept your fate in hellfire."

# game/script.rpy:681
translate english for_2e1ebdad:

    # a "Нет! Чертов урод! Давай же, поддавайся."
    a "No! Damn freak! Come on, yield."

# game/script.rpy:690
translate english for_43c8bd45:

    # leo ghost vspom "Я не должен жить."
    leo ghost vspom "I shouldn't have to live."

# game/script.rpy:691
translate english for_ea3d59de:

    # leo "Я пытался... уйти... но эти двое так и не дали мне довести ту попытку до конца."
    leo "I tried to leave but these two never let me do it"

# game/script.rpy:692
translate english for_a64b37d6:

    # leo "Мне пришлось жить и мучаться, жить и каждый день ненавидеть себя."
    leo "I had to live and suffer, live, and despise myself every day."

# game/script.rpy:693
translate english for_d83d6de9:

    # leo "Ты можешь себе представить, каково это?"
    leo "Can you imagine what it would be like?"

# game/script.rpy:694
translate english for_cb2c537f:

    # a "Дед, я сейчас обращаюсь к тебе."
    a "Grandfather, I'm talking to you now."

# game/script.rpy:695
translate english for_caf7c51a:

    # a "Хватит. Больше незачем прикидываться кем-то другим."
    a "Enough. There is no need to pretend to be someone else."

# game/script.rpy:696
translate english for_65e247bf:

    # a "Прошу, прими уже ответственность за все."
    a "Please, take responsibility for everything."

# game/script.rpy:697
translate english for_9a021e37:

    # leo ghost "Ты не понимаешь..."
    leo ghost "You don't understand..."

# game/script.rpy:698
translate english for_d5a91fca:

    # leo "Я не могу. Слишком велика моя вина."
    leo "I can't. My guilt is too great."

# game/script.rpy:699
translate english for_52d62a65:

    # leo "Мне нет прощения.{w} Я ведь убил мою Сару... сам убедись, как это было ужасно."
    leo "There is no forgiveness. I’ve killed my Sarah... behold how awful it was."

# game/script.rpy:705
translate english for_363e527b:

    # s "А ну, не смей поддаваться!"
    s "Well, don't you dare give in!"

# game/script.rpy:706
translate english for_2d008f9c:

    # s "Ну же, защищайся достойно! Я хочу поразить тебя в честной борьбе."
    s "Come on, defend yourself with dignity! I want to impress you in a fair fight."

# game/script.rpy:707
translate english for_e6bd353c:

    # d "Ты уже поразила меня однажды, любовь моя, и теперь ты всегда здесь, в моем сердце."
    d "You’ve already struck me once, my love, and now you are always here in my heart."

# game/script.rpy:708
translate english for_d6b3a2e1:

    # s "Ах, ты невыносимо галантен."
    s "Ah, you are unbearably gallant."

# game/script.rpy:709
translate english for_0fbd6bab:

    # s "Я так рада, что ты снова счастлив. Как дела с твоей новой книгой?{w} Верю, у тебя все получится!"
    s "I am so glad that you are happy again. How are you doing with your new book? I believe you will succeed!"

# game/script.rpy:711
translate english for_f8070717:

    # s "Почему ты молчишь? Что не так?"
    s "Why are you silent? What's wrong?"

# game/script.rpy:712
translate english for_0f6b821c:

    # d "Ты издеваешься надо мной?{w} Как я могу написать хотя бы строчку чего-то стоящего, если я все время провожу с тобой?"
    d "Are you kidding me? How can I write even a line of something worthwhile if I spend all my time with you?"

# game/script.rpy:713
translate english for_44e4f551:

    # s "Что ты такое говоришь? Я не требую твоего внимания постоянно!"
    s "What are you saying? I don't need your attention all the time!"

# game/script.rpy:714
translate english for_d2a0f07b:

    # s "Николас, ты что перестал принимать препараты? Тебе же помогала терапия!"
    s "Nicholas, have you stopped taking your meds? Therapy helped you!"

# game/script.rpy:715
translate english for_9fcacf74:

    # d "Ты... ты такая же как они все. Ты хочешь сделать из меня послушную марионетку? Напичкать таблетками?"
    d "You... you are just like them all. Do you want to make me an obedient puppet? Stuffed with pills?"

# game/script.rpy:716
translate english for_93dbf3ea:

    # s "Николас, милый, нет, это не ты, не ты говоришь... сопротивляйся ему, прошу!"
    s "Nicholas, honey, no! It is not you, it’s not your words resist him, please!"

# game/script.rpy:717
translate english for_81498b78:

    # g "Ты... это ты портишь все мои замыслы!{w} Смеешься втайне надо мной!"
    g "You... you ruined all my plans! Mock at me!"

# game/script.rpy:718
translate english for_d1a1dd94:

    # g "Хочешь прибрать к рукам мой дом? Уже сговорилась со своими грязными дружками?"
    g "Do you want to take my house? Already conspired with your filthy hippie friends?"

# game/script.rpy:719
translate english for_47d439cf:

    # g "Лживая шлюха, я убью тебя!"
    g "Deceitful slut, I’ll kill you!"

# game/script.rpy:721
translate english for_a1aab7d9:

    # leo "Теперь ты понимаешь? Она верила в меня. Она единственная поддерживала меня. До конца."
    leo "Do you understand now? She believed in me. She was my only support. To the end."

# game/script.rpy:722
translate english for_4d690a89:

    # leo "Я никогда не смогу простить себя."
    leo "I can never forgive myself."

# game/script.rpy:727
translate english for_242cb4e7:

    # s "Ты не виноват, и я тебя никогда не винила."
    s "You are not to blame and I never blamed you."

# game/script.rpy:728
translate english for_1e6c2b7a:

    # s "Это был не ты, а твой недуг, выпестованный жестоким миром. Ты помнишь, как много боли тебе нанесли?"
    s "IIt was not you, but your disease, fostered by the cruel world. Do you remember how much pain they inflicted on you?"

# game/script.rpy:729
translate english for_4dcb1c29:

    # leo "Сара... я был слишком слаб.{w} Я должен был справиться..."
    leo "Sarah... I was too weak. I had to handle..."

# game/script.rpy:730
translate english for_a3a8b5a1:

    # s "Милый, ты не мог этим управлять. Прими это."
    s "Honey, you couldn't control it. Admit it."

# game/script.rpy:731
translate english for_24f522b3:

    # s "Ты был сильным и сделал все что мог. Именно такого сильного человека я полюбила, такого я хочу сейчас."
    s "You were strong and did everything you could. The strong person I fell in love with and still want to stand beside."

# game/script.rpy:732
translate english for_5a71667d:

    # s "Пойдем со мной..."
    s "Come with me..."

# game/script.rpy:735
translate english for_3cc4a01e:

    # d "Я рад тебя вновь видеть. Ты... возмужал."
    d "I'm glad to see you again. You... matured."

# game/script.rpy:736
translate english for_dcf62ca2:

    # d "Я горжусь тобой, Артур. Ты спас меня и сделал то, чего не смог я за всю свою жизнь."
    d "I'm proud of you, Arthur. You saved me and did what I couldn't in my whole life."

# game/script.rpy:737
translate english for_558f7444:

    # d "Я знаю, я никогда не смогу по достоинству отблагодарить тебя. Прости меня за все."
    d "I know, I can never truly thank you. Forgive me for everything."

# game/script.rpy:740
translate english for_68b367d1:

    # d "Вот, я отдаю эти документы. Теперь ты - хозяин этого дома. И я хочу отдать тебе свою последнюю книгу, посмертный труд."
    d "Here, I am giving these documents. Now you are the owner of this house. And I want to give you my last book, posthumous work."

# game/script.rpy:741
translate english for_584de211:

    # d "Распорядись этим так, как сочтешь нужным. Ты доказал, что умеешь принимать мудрые решения."
    d "Arrange it as you see fit. You have proven that you can make wise decisions."

# game/script.rpy:742
translate english for_3dd28236:

    # d "А теперь позволь мне уйти с любимой."
    d "Now let me leave with my beloved."

# game/script.rpy:747
translate english for_d6e40d48:

    # leo ghost vspom "Спасибо тебе. Без тебя я никогда не решился бы."
    leo ghost vspom "Thank you. Without you, I would never have dared."

# game/script.rpy:748
translate english for_7253196b:

    # leo "Спасибо, что никто из тех двоих мне больше не мешает уйти."
    leo "Thank you. Without you, I would never have dared."

# game/script.rpy:749
translate english for_844623df:

    # leo "Я так и не смог обрести целостность, но я и не заслужил."
    leo "I have failed to find integrity, but I did not deserve it anyway."

# game/script.rpy:750
translate english for_90c50654:

    # leo "Впервые я чувствую облегчение. Я не знаю, сможешь ли ты простить меня... нас."
    leo "For the first time I feel relief. I don't know if you can forgive me... forgive us."

# game/script.rpy:751
translate english for_7016bc23:

    # leo "Возьми это:"
    leo "Take this:"

# game/script.rpy:755
translate english for_d1bb609a:

    # leo "Это место больше не держит тебя. Ты свободен. Как и я."
    leo "This place no longer holds you. You are free. As I am."

# game/script.rpy:759
translate english for_d85dbad9:

    # leo "Прошу тебя, избавься от этого дома. Найди свой путь и свое место в мире."
    leo "lease, get rid of this house. Find your path and your place in the world."

# game/script.rpy:760
translate english for_304b2541:

    # s "Как же долго я ждала этого пиршества. И вот оно наступило."
    s "I have been awaiting this feast for so long. And it finally came."

# game/script.rpy:761
translate english for_78476522:

    # s "Хотя из-за твоего несмышленого внука я так и не получила всего Николаса, но и ты сгодишься, сладкий."
    s "Although I’ll never get all of Nicholas because of your foolish grandson, you are fine too, honey."

translate english strings:

    # game/script.rpy:286
    old "Надо внимательнее исследовать кабинет"
    new "I should examine the cabinet more carefully."

    # game/script.rpy:286
    old "Будет лучше пойти еще дальше по коридору второго этажа"
    new "I’d better go along the corridor on the first floor."

    # game/script.rpy:375
    old "... подтверждает расщепление личности на фоне тяжелого депрессивного расстройства. Вероятно, генезисом или триггером данного феномена послужила травля новых книг пациента в прессе, кроме того, на нескольких сеансах пациент упоминал смерть некоего близкого человека, которая, по его мнению, произошла по его вине. Временами говорит прямо «я убил», иногда «мы убили», иногда «он убил» - что это, навязчивые диссоциативные галлюцинации? Пациент, однако, отказывается углубляться в подробности."
    new "... it confirms the personality fragmentation amid a severe depressive disorder. The genesis or the trigger for this phenomenon is likely to have been a harsh criticism that patient’s books faced in the media. Therewith, in some sessions the patient mentioned the death of some close person caused, in his view, by his fault. Sometimes he said «I killed» or «we killed»,  quite rarely - «he killed». Can it be a symptom of dissociative hallucinations? However, the patient refused to delve into details..."

    # game/script.rpy:377
    old "Очевидно, что субличности пациента играют роль гипертрофированных защитных барьеров психики. Так, Уильям - это, несомненно, идеальный образ себя как автора, способного творить выдающиеся произведения искусства. Грегори защищает хрупкое эго автора от нападок критики и, в целом, внешней среды. Но какую функцию играет редко появляющийся Лайонель? Пока неясно. Надеюсь, увеличение дозы антидепрессантов поможет избежать повторения попыток суицида и параноидальных приступов маломотивированной агрессии..."
    new "Undoubtedly, the patient’s subpersonalities serve as hypertrophied protective barriers. Apparently, William is a perfect image of himself as a writer able to create distinguished masterpieces. Gregory defends his fragile ego from criticism and aggressive environment. But what functions does rarely emerging Lionel fulfil? It’s unclear. I hope, increased dose of ADs will help to at least avoid new suicide attempts and paranoid unprovoked aggression..."

    # game/script.rpy:387
    old "Ты знаешь Грегори или Лайонеля?"
    new "Do you know Gregory or Lionel?"

    # game/script.rpy:387
    old "Ты знаешь что-то про девушку с портрета внизу на каминной полке?"
    new "Do you know anything about a girl depicted in the picture in the living room?"

    # game/script.rpy:401
    old "Ты кто?"
    new "Who are you?"

    # game/script.rpy:423
    old "Ты не в курсе, Николас никого не убивал?"
    new "Do you know if Nicholas killed somebody?"

    # game/script.rpy:423
    old "Тебе заняться больше нечем, как сидеть тут и заниматься «интеллектуальной деятельностью» в темноте?"
    new "You really have nothing to do but to sit here in shadows and do your pointless «intellectual activity», don’t you?"

    # game/script.rpy:423
    old "Хочешь, расскажу тебе то, что ты не осознаешь?"
    new "What if I tell you something that you never realize on your own?"

    # game/script.rpy:423
    old "Вы не знаете, Николас дописал перед смертью ту книгу, про которую делал объявление в прессе?"
    new "Do you happen to know whether y Nicholas has finished writing his last book?"

    # game/script.rpy:442
    old "Я - внук Николаса и его прямой наследник. Эта книга теперь принадлежит мне. Прошу отдать ее."
    new "I’m a grandson of Nicholas and his heir. This book belongs to me. Please, give it."

    # game/script.rpy:442
    old "Почему вы все время оскорбляете моего деда? Он тоже был великим писателем. И вы еще называете себя его другом?"
    new "Why are you constantly insulting my grandad? He was a famous writer! And you dare call yourself his friend?"

    # game/script.rpy:471
    old "Получена книга вашего деда"
    new "The book of your grandfather received."

    # game/script.rpy:566
    old "Ублюдок, ты не смеешь так со мной разговаривать! А ну пошел прочь!"
    new "You bastard, you dare not speak with me in this way! Now, go to hell!"

    # game/script.rpy:566
    old "Эта шпага кого-то уже отнимала чью-то жизнь?"
    new "This sword has already taken someone’s life?"

    # game/script.rpy:581
    old "Уклониться"
    new "Dodge the sword strike"

    # game/script.rpy:592
    old "Я - внук и законный наследник Николаса. Я пришел за своим."
    new "I am a grandson of Nicholas and his legitimate heir. I’ve come for what is mine."

    # game/script.rpy:592
    old "И что, хорошо ты защитил моего деда, раз он страдал до самой смерти?"
    new "So, you guarded my grandad well, didn’t you? So well, that he suffered from pain and solitude till death."

    # game/script.rpy:599
    old "Получены документы на дом"
    new "Mansion deeds received"

    # game/script.rpy:666
    old "Перебить"
    new "Interrupt"

    # game/script.rpy:666
    old "Не вмешиваться"
    new "Stay out"

    # game/script.rpy:738
    old "Получены документы на дом и книга"
    new "Mansion deeds and book received"

    # game/script.rpy:753
    old "Получен ключ"
    new "Key received"

