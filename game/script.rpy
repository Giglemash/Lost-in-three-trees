...
# scene Фото на камине
    ...
    a "Похоже, сплетни не врали. Старый развратник. На вид она ничего так..."
    a "Не ради ли тебя наш дед забыл о нашей семье? Хотя... если он так легко выкинул нас из сердца, то проблема тут явно не в девушке..."
...
# Тяжелое дыхание
    ...
    a "Все не могу понять: я брежу? Я сплю?{w} Или с этим местом что-то не так?"
    a "Они же выглядят неживыми и...{w} гниющими?"
...
# Письмо
    ...
    a "Похоже, и вправду попал в дом с привидениями. Или в готический роман. Эдгар, это ты балуешься?"
...
# Слышен звук скрипящего пера.
    ...
    if sc10one:
        ...
        while len(q) < 3:
            menu:
                ...
                "Ты не в курсе, Николас никого не убивал?" if not 3 in q:
                    $ q.append(3)
                    $ renpy.block_rollback()
    ...
    scene kabinet
    menu:
        "Тебе заняться больше нечем, как сидеть тут и заниматься «интеллектуальной деятельностью» в темноте?":
